...
public class ShotTimingTunerOpMode extends LinearOpMode {
    ...
    while (opModeIsActive()) {
      ...
      if (shotRunning) {
        ...
        if (ballTimesMs.size() >= expectedBalls) {
          shotRunning = false;
          status = String.format("all %d out", expectedBalls);
        } else if (elapsedMs >= shotBudgetMs()) {
          shotRunning = false;
          status =
              String.format(
                  "TIMED OUT with %d of %d — pull the csv", ballTimesMs.size(), expectedBalls);
        }
  ...
  private void render(
    ...
    if (!flywheelIdling) {
      ...
      flywheel =
          String.format(
              "SPINNING UP  %.0f%%", robot.shotController.getFlywheelVelocityRatio() * 100);
    }
    telemetry.addData("FLYWHEEL", flywheel);

    telemetry.addData(
        "SHOT", shotRunning ? String.format("FIRING %d ms", elapsedMs) : "idle — " + status);
      ...
      for (TimingPoint p : trials) {
        telemetry.addLine(
            String.format(
                ...
                p.distanceInches(), p.shootWindowMs(), p.ballCount(), p.firstBallMs()));
      }
      telemetry.addLine("========================================");
      telemetry.addData("Trace rows", trace.size());
      telemetry.addLine("/sdcard/FIRST/teamcode/shot_time_table.yaml");
      telemetry.addLine("/sdcard/FIRST/teamcode/shot_timing_log.csv");
      telemetry.addData(
  ...
  private void saveProgress() {
    ...
    sorted.sort((a, b) -> Double.compare(a.distanceInches(), b.distanceInches()));
    saveTable(sorted);
    saveTrace();
  }
    ...
    try (FileWriter writer = new FileWriter(new File(dir, "shot_time_table.yaml"), false)) {
      ...
      for (int i = 0; i < points.size(); i++) {
        ...
        writer.write(
            String.format(
                "        %.1f, %d%s%n",
                p.distanceInches(), p.shootWindowMs(), (i == points.size() - 1) ? "" : ","));
      }
      writer.write("      ]\n");
