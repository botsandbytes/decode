...
public final class config {
  ...
  public static int AUTO_ENDGAME_PERIOD_MS;
  public static int AUTO_ENDGAME_EVACUATE_MS;
  public static int AUTO_ENDGAME_FREEZE_MS;
  public static double AUTO_ENDGAME_EXIT_CLEARANCE;
  ...
  public static synchronized void reload() {
    ...
    AUTO_ENDGAME_PERIOD_MS = auto.endgame.period_ms;
    AUTO_ENDGAME_EVACUATE_MS = auto.endgame.evacuate_ms;
    AUTO_ENDGAME_FREEZE_MS = auto.endgame.freeze_ms;
    AUTO_ENDGAME_EXIT_CLEARANCE = auto.endgame.exit_clearance;
  ...
  public static final class Turret {
    ...
    /**
     * Flips which CRServo direction counts as increasing turret angle. Set this (not the analog encoder's 'inverted' flag) when the turret drives away from its target.
     */
    public boolean servo_direction_inverted;
    /**
     * Static friction feedforward (kS) magnitude applied when turning turret in positive direction (error > 0).
     * Minimum: 0.0
     * Maximum: 1.0
     */
    public double ks_positive;
    /**
     * Static friction feedforward (kS) magnitude applied when turning turret in negative direction (error < 0).
     * Minimum: 0.0
     * Maximum: 1.0
     */
    ...
    public static final class Travel {
      /**
       * Most negative turret angle (degrees, relative to the chassis) the turret is allowed to reach.
       * Minimum: -180.0
       * Maximum: 0.0
       */
      public double min_angle;
      /**
       * Most positive turret angle (degrees, relative to the chassis) the turret is allowed to reach.
       * Minimum: 0.0
       * Maximum: 180.0
       */
    ...
    public static final class Stall {
      ...
      public boolean enabled;
      /**
       * Commanded turret power above which the stall watchdog starts checking for motion. Below this the servo may legitimately not move, so no stall is inferred.
       * Minimum: 0.0
       * Maximum: 1.0
       */
      public double power_threshold;
      /**
       * Turret motion (degrees) that counts as progress and resets the stall watchdog. Set above the analog encoder's noise floor.
       * Minimum: 0.0
       */
      public double min_progress_deg;
      /**
       * Seconds of commanded power with no turret motion before the stall watchdog latches a fault and cuts power. Protects the turret from being driven into a hard stop when the encoder reading is wrong.
       * Minimum: 0.0
       */
    ...
    public static final class Runaway {
      ...
      public boolean enabled;
      /**
       * How far (degrees) the turret's aim error may grow beyond its best-so-far value before the runaway watchdog cuts power. Catches an inverted control loop, where positive power moves the turret away from its target, before it reaches a hard stop.
       * Minimum: 0.0
       */
    ...
    public static final class AnalogEncoder {
      ...
      public boolean enabled;
      /**
       * Analog voltage (V) corresponding to 0 degree turret angle position.
       * Minimum: 0.0
       * Maximum: 3.3
       */
      public double zero_voltage;
      /**
       * Scale factor (degrees/volt) converting analog input voltage delta to turret angle in degrees.
       */
      public double degrees_per_volt;
      /**
       * Minimum safe voltage limit for the analog encoder.
       * Minimum: 0.0
       * Maximum: 3.3
       */
      public double min_voltage;
      /**
       * Maximum safe voltage limit for the analog encoder.
       * Minimum: 0.0
       * Maximum: 3.3
       */
      public double max_voltage;
      /**
       * Full electrical span (volts) of the turret potentiometer, i.e. the voltage at which it wraps back to zero.
       * Minimum: 0.0
       */
  ...
  public static final class Sentinel {
    ...
    /**
     * Time horizon (seconds) used to project future rotation based on live angular velocity (|omega| * time). Seeded at 0.075s (derived from 0.45rad / assumed 6.0rad/s max omega); calibrate on robot using Friction Calibration TeleOp.
     * Minimum: 0.0
     */
  ...
  public static final class Casablanca {
    /**
     * Enable/disable depth (X-axis) goal zone proximity protection. Safety policy parameter; leave true outside of debugging.
     */
    public boolean enable_depth_protection;
    /**
     * Enable/disable side (Y-axis) goal zone proximity protection. Safety policy parameter; leave true outside of debugging.
     */
    public boolean enable_side_protection;
    public static final class Repulsion {
      /**
       * Motor power (0.0 to 1.0) commanded to push the robot away when it violates a goal zone boundary. Driver/safety policy parameter.
       * Minimum: 0.0
       * Maximum: 1.0
       */
      public double power;
      /**
       * Safety margin factor (0.0 to 1.0) for predictive braking (e.g. 0.7 = 30% safety margin over physics model prediction). Risk tolerance/policy parameter.
       * Minimum: 0.0
       * Maximum: 1.0
       */
    ...
    public static final class Depth {
      /**
       * Distance (inches) from goal zone at which proximity speed reduction starts. Driver comfort / early-warning feel parameter.
       * Minimum: 0.0
       */
      public double slow_down;
      /**
       * Distance (inches) from goal zone where drive power towards zone is cut to zero. Safety policy parameter.
       * Minimum: 0.0
       */
    ...
    public static final class HeadingLock {
      ...
      public boolean enabled;
      /**
       * Raw joystick turn threshold (0.0 to 1.0) above which driver active steering takes control and disengages heading lock.
       * Minimum: 0.0
       * Maximum: 1.0
       */
      public double intent_threshold;
      /**
       * Minimum rotational drive power (0.0 to 1.0) needed to overcome friction while the robot is already translating.
       * Minimum: 0.0
       * Maximum: 1.0
       */
      public double ks_moving;
      /**
       * Chassis translational speed threshold (inches/sec) above which full ks_moving friction feedforward is applied.
       * Minimum: 0.0
       */
      ...
      /**
       * Heading error (degrees) below which heading lock applies zero correction, to avoid chattering/jitter from sensor noise while holding a heading at rest.
       * Minimum: 0.0
       * Maximum: 15.0
       */
      public double error_deadband_deg;
      /**
       * Rotation rate (degrees/sec) below which the robot counts as settled. Heading lock waits for this before capturing the heading to hold, so it does not latch mid-coast and fight the robot's own momentum.
       * Minimum: 0.0
       */
  ...
  public static final class Shooter {
    /**
     * When true, uses the REV firmware velocity PID on the motor (RUN_USING_ENCODER with setVelocity, gains from shooter.motor_pidf). When false, uses the custom Pedro PIDF controller, anti-windup integrator, and voltage compensation (RUN_WITHOUT_ENCODER, gains from shooter.pidf).
     */
    public boolean use_ftc_pid;
    public double ks;
    /**
     * Bus voltage the flywheel feedforward (ks and pidf.f) was characterized at. Shooter.periodic() scales its motor command by nominal_voltage divided by the measured bus voltage, so a sagging pack still reaches the commanded wheel speed instead of quietly landing short and failing the feed gate. Set this to the voltage shown while the Shooter Characterization OpMode is running, not to a fresh-off-the-charger reading.
     * Minimum: 1.0
     * Maximum: 20.0
     */
    public double nominal_voltage;
    /**
     * Largest factor the flywheel voltage compensation may scale its command by. Bounds the correction so a brownout or a bad voltage reading cannot rail the flywheel; 1.4 covers a 12.0 V feedforward down to about 8.6 V.
     * Minimum: 1.0
     * Maximum: 3.0
     */
    public double max_voltage_compensation;
    /**
     * How often the flywheel re-reads bus voltage. The hub's voltage is a separate ADC round trip rather than part of the bulk read, so polling it every loop costs milliseconds per tick; a pack does not sag meaningfully inside this interval.
     * Minimum: 0.0
     * Maximum: 5.0
     */
    ...
    /**
     * Continuous fixed flywheel target speed (RPM). Flywheel holds this RPM constantly so time-to-shoot collapses to alignment time.
     * Minimum: 0.0
     * Maximum: 3000.0
     */
    ...
    /**
     * Flywheel velocity upper bound scale factor (e.g. 1.05) to prevent overshooting before feeding rings. Applies only when arming a feed, not while one is already running, since a ring can only ever slow the flywheel down.
     * Minimum: 1.0
     * Maximum: 2.0
     */
    public double max_velocity_threshold;
    /**
     * Fraction of target flywheel speed a feed already in progress may sag to before the intake cuts out. Must be below min_transfer_threshold: it is the lower half of a hysteresis band, so each ring entering the flywheel does not drop the speed under the arming floor and stutter the intake off and back on. Raise it toward min_transfer_threshold for stricter shot energy, lower it if the intake still hitches while feeding.
     * Minimum: 0.0
     * Maximum: 1.0
     */
    public double feed_release_threshold;
    /**
     * Front intake power (0.0 to 1.0) when feeding rings to the shooter.
     * Minimum: 0.0
     * Maximum: 1.0
     */
    public double feed_intake_power;
    /**
     * Transfer motor power (0.0 to 1.0) when feeding rings to the shooter.
     * Minimum: 0.0
     * Maximum: 1.0
     */
    public double feed_transfer_power;
    /**
     * Launch power ratio threshold above which the high hood position is selected for long-range shots.
     * Minimum: 0.0
     * Maximum: 1.0
     */
    ...
    public static final class BallDetection {
      /**
       * How far flywheel speed must fall below its running reference, as a fraction, to count as a ball leaving the shooter. Measured on this robot: real balls dip 12.2%-26.4%, the deepest non-ball (the wheel coasting down after a shot) reaches 5.4%. Keep this between them; see FlywheelDipDetector.
       * Minimum: 0.0
       * Maximum: 1.0
       */
      public double dip_fraction;
      /**
       * How far flywheel speed must climb back up off a dip's own trough, as a fraction, before the next ball can register. Must be strictly below dip_fraction. Ending a dip on a rebound off the trough — not on a return to the pre-ball reference — is what lets balls that arrive close together each get counted; see FlywheelDipDetector.
       * Minimum: 0.0
       * Maximum: 1.0
       */
      public double rebound_fraction;
      /**
       * EMA weight per control loop for the ball detector's running reference speed. Small enough that one ball cannot drag the reference down with it, large enough to track a flywheel settling into a new cruise speed between shots.
       * Minimum: 0.0
       * Maximum: 1.0
       */
      public double baseline_alpha;
      /**
       * Minimum spacing (milliseconds) between counted ball events. One ball produces a dip with a ragged floor; without this, noise on the way back up would count as a second ball.
       * Minimum: 0.0
       */
    ...
    public com.qualcomm.robotcore.hardware.PIDFCoefficients pidf;
    /**
     * REV firmware velocity PIDF (p, i, d, f) applied directly to the flywheel motors via DcMotorEx.setPIDFCoefficients when shooter.use_ftc_pid is true. Distinct from shooter.pidf, which only feeds the software PIDFController used when use_ftc_pid is false and is never sent to the motor. Keep i at 0 here: a live integral on the motor firmware winds up during a feed's velocity sag and overshoots the setpoint once the ball leaves, which is what stalled the third shot of a magazine.
     */
    public com.qualcomm.robotcore.hardware.PIDFCoefficients motor_pidf;
    public static final class Integral {
      /**
       * Flywheel velocity error (ticks/s) within which the integral term accumulates. Outside this band the integrator holds its value, so spin-up and ball-feed transients never charge it.
       * Minimum: 0.0
       * Maximum: 2000.0
       */
      public double band_ticks;
      /**
       * Maximum absolute motor power the flywheel integral term may contribute. Bounds windup independently of the integral gain.
       * Minimum: 0.0
       * Maximum: 1.0
       */
    ...
    public static final class Ballistics {
      /**
       * Fitted initial ring exit velocity (in/s) at constant RPM.
       * Minimum: 0.0
       */
      public double v0;
      /**
       * Fitted lumped quadratic drag coefficient (1/in) for 2D trajectory integration.
       * Minimum: 0.0
       */
      public double k;
      /**
      ...
      /**
       * Physical ring release height (inches) above field carpet floor.
       * Minimum: 0.0
       */
      public double launch_height_inches;
      /**
       * Center height (inches) of target scoring goal basket above field carpet floor.
       * Minimum: 0.0
       */
      public double goal_height_inches;
      /**
       * Gravitational acceleration constant (in/s²).
       * Minimum: 0.0
       */
      public double g_inches_per_sec2;
      /**
       * Minimum physical hood elevation angle (degrees).
       * Minimum: 0.0
       * Maximum: 90.0
       */
      public double min_hood_angle_deg;
      /**
       * Maximum physical hood elevation angle (degrees).
       * Minimum: 0.0
       * Maximum: 90.0
       */
      public double max_hood_angle_deg;
      /**
       * Continuous servo position (0.0 to 1.0) corresponding to min_hood_angle_deg, the flattest arc. The hood linkage is reversed, so this is numerically GREATER than max_hood_servo_pos; that is expected and the angle mapping handles it. Do not 'fix' the ordering.
       * Minimum: 0.0
       * Maximum: 1.0
       */
      public double min_hood_servo_pos;
      /**
       * Continuous servo position (0.0 to 1.0) corresponding to max_hood_angle_deg, the steepest arc. Servo 0.0 is as tall as the arc gets, so this is normally the smaller number of the pair.
       * Minimum: 0.0
       * Maximum: 1.0
       */
      public double max_hood_servo_pos;
      /**
       * Maximum allowed robot translation speed (in/s) for valid moving shots.
       * Minimum: 0.0
       */
      public double max_moving_speed_ips;
      /**
       * Minimum calibrated shot distance (inches) for valid solution.
       * Minimum: 0.0
       */
      public double min_valid_distance;
      /**
       * Maximum calibrated shot distance (inches) for valid solution.
       * Minimum: 0.0
       */
      public double max_valid_distance;
      /**
       * Empirical calibration multiplier for virtual lead target calculation.
       * Minimum: 0.0
       */
      public double lead_bias_gain;
      /**
       * Flywheel setpoint the fitted v0 exit velocity was measured at. Exit speed and flywheel setpoint convert through v0 / v0_reference_rpm, which is how the solver turns a required exit speed into an RPM. Must match the RPM the ballistics calibration actually ran at (constant_rpm), or every solved RPM is scaled wrong.
       * Minimum: 1.0
       * Maximum: 3000.0
       */
      public double v0_reference_rpm;
      /**
       * Flywheel setpoint the hood aims around. The solver holds this speed and lets the hood alone cover every distance it can, so the flywheel stays parked at one well-calibrated, already-spun-up speed. It only rises above this when no hood angle can reach the goal, and never falls below it. This is the main shot-strength knob: raise it if shots land weak across the board.
       * Minimum: 0.0
       * Maximum: 3000.0
       */
      public double preferred_shot_rpm;
      /**
       * Highest flywheel setpoint the RPM solver may command, for distances past the hood's authority at preferred_shot_rpm. A target that cannot reach goal height even at this speed is reported as an invalid solution instead of being silently under-shot.
       * Minimum: 0.0
       * Maximum: 3000.0
       */
      public double max_shot_rpm;
      /**
       * Headroom above the slowest exit speed that still reaches the goal (0.12 = 12% faster), applied only to long shots that had to raise RPM past preferred_shot_rpm. Keeps those solutions off the boundary where the flat and lofted trajectory branches merge; larger values give a steeper, more forgiving entry angle at the cost of a hotter shot.
       * Minimum: 0.0
       * Maximum: 1.0
       */
      public double v0_margin_fraction;
      /**
       * Minimum change in solved RPM (ticks/s) before the flywheel setpoint is re-commanded. Every setpoint write discards the flywheel's accumulated anti-windup integral, so without a deadband a continuously drifting target would keep the integrator reset and the flywheel permanently short of speed.
       * Minimum: 0.0
       */
      public double rpm_update_deadband;
      /**
       * Ring time of flight per inch of range, used only to lead a moving shot. A flat estimate on purpose: the measured shot table records hood and RPM but not flight time, and lead only shifts the aim azimuth slightly, so a rough value is adequate here where a wrong hood position would not be.
       * Minimum: 0.0
       * Maximum: 0.1
       */
      public double flight_time_sec_per_inch;
      /**
       * Robot speed below which moving-shot lead is skipped entirely. A robot holding position still reports a small noisy velocity, and lead turns that noise into aim-azimuth jitter larger than the turret's settle tolerance, so the turret hunts instead of settling. Leading a nearly-stationary robot gains nothing.
       * Minimum: 0.0
       * Maximum: 100.0
       */
    ...
    public static final class ShotTable {
      /**
       * Measured shot table as flat distance_in, hood_servo, flywheel_rpm triples, recorded by the Ballistics Calibration OpMode. Distances must strictly increase. Looked up by linear interpolation between rows and clamped outside them; a distance outside the calibrated range is reported as an invalid solution rather than extrapolated.
       */
  ...
  public static final class Auto {
    /**
     * Measured shoot windows as flat target_rpm, shoot_window_ms pairs, recorded by the Shot Timing Tuner OpMode against whatever flywheel RPM the shot table commanded at each measured distance. RPMs must strictly increase. Looked up by linear interpolation on the flywheel's target RPM and clamped outside the measured range, since a shot just past the last row does not suddenly get faster. Keying on RPM rather than distance means the table stays correct after shooter.shot_table is recalibrated — the shot only ever behaved the way it did at the RPM it was actually fired at, not at whatever distance a recalibrated table now maps that RPM to. ShotController ends a scoring cycle when it counts auto.balls_per_shot_count balls OR this window expires, whichever comes first — the window is the safety net for a jam, not the primary signal. auto.shoot_wait_ms is only the fallback when this table is empty.
     */
    public double[] shot_time_points;
    /**
     * Balls loaded before each autonomous scoring cycle. ShotController counts them live via shooter.ball_detection and ends the shot the moment this many are detected, without waiting out the rest of auto.shot_time_points' window.
     * Minimum: 1.0
     */
    ...
    /**
     * Fallback wait time (milliseconds) allowed to fire a shot, used only when auto.shot_time_points is empty. Prefer the measured table: a single flat number cannot express that a close-range magazine takes longer than a long-range one.
     * Minimum: 0.0
     */
    ...
    public static final class Endgame {
      /**
       * Length of the autonomous period (milliseconds), measured from the moment the OpMode starts. The endgame timers below are all counted backwards from it.
       * Minimum: 0.0
       */
      public int period_ms;
      /**
       * Time left in autonomous (milliseconds) at which the auto sequence is cancelled and the robot drives the shortest straight line to a spot where it is committed — entirely inside a launch zone or entirely outside every one. A robot straddling a zone boundary at the buzzer gets neither the zone's benefit nor out of the next alliance's way.
       * Minimum: 0.0
       */
      public int evacuate_ms;
      /**
       * Backstop deadline (milliseconds left in autonomous) at which the robot stops driving even if it never reached a committed spot. Normally motion stops earlier, on arrival. Must be smaller than auto.endgame.evacuate_ms, which is the window the move gets to finish in.
       * Minimum: 0.0
       */
      public int freeze_ms;
      /**
       * Inches of commitment required on whichever side of the launch zone boundary the robot parks on: its whole footprint must clear the line by this much, inside or out. Absorbs the follower's settling error so a spot that ends slightly short is still unambiguous. Measured against the footprint's half-diagonal, so it holds at any heading.
       * Minimum: 0.0
       */
      public double exit_clearance;
    }
  ...
  public static final class Teleop {
    ...
    /**
     * When true, driver translation sticks are interpreted in the field frame (stick direction is a fixed field direction regardless of robot heading). When false, sticks are robot-relative.
     */
    public boolean field_centric;
    /**
     * Field heading (degrees) that a fully-forward driver stick should drive toward when field_centric is true. 90 means stick-forward drives along field +Y, i.e. away from the driver wall at y=0. Set this to whatever direction is 'away' from where the drivers physically stand.
     * Minimum: -360.0
     * Maximum: 360.0
     */
    public double field_centric_offset_deg;
    /**
     * Target motor power (0.0 to 1.0) for intake rollers in TeleOp.
     * Minimum: 0.0
     * Maximum: 1.0
     */
    ...
    public double transfer_power;
    /**
     * Target flywheel power ratio (0.0 to 1.0) for manual rev command in TeleOp.
     * Minimum: 0.0
     * Maximum: 1.0
     */
